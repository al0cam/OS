...
{  /* dobavi skup semafora sa ukupno n semafora */
   SemId = semget(IPC_PRIVATE, n, 0600);//3 0 1 2
   if (SemId == -1) {
...
{  /* postavi vrijednost semafora SemNum na SemVal */
   return semctl(SemId, SemNum, SETVAL, SemVal);//1 10
}

int SemGetVal(int SemNum)
{  /* citaj vrijednost semafora SemNum na SemVal */
   return semctl(SemId, SemNum, GETVAL, NULL);//1 vraca 10
}
...
{  /* obavi operaciju SemOp sa semaforom SemNum */
   ...
   SemBuf.sem_op  = SemOp;
   SemBuf.sem_flg = 0;
   return semop(SemId, &SemBuf, 1);//1 1 11 -1 9
}
...
class eko
{
    ...
    void potrosac()
    {
        ...
        do
        {
            ...
            while(1)
            {
                while (s->brojac<broj_prodavaca_global)
                {
                    if(SemGetVal(s->brojac)) break;
                    s->brojac=(s->brojac+1)%broj_prodavaca_global;
                }
                if(SemGetVal(s->brojac)) break;
            }
            int j=s->brojac;
            ...
            SemOp(j,-1);
        } while (i++<(broj_proizvoda_global*broj_prodavaca_global)-1);
